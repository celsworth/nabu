...
// english language modifications by chrise
//  * dropped 'about'
//  * 1 minute, 1 hour -> a minute, an hour; etc
//  * support data-suffix in the <time> element for a custom suffix
  ...
  TimeAgo = (function() {
    ...
    TimeAgo.prototype.updateTime = function() {
      ...
      return this.$element.findAndSelf(this.options.selector).each(function() {
        var timeAgoInWords;
        timeAgoInWords = self.timeAgoInWords($(this).attr(self.options.attr), $(this).attr('data-suffix'));
        return $(this).html(timeAgoInWords);
      });
    };

    TimeAgo.prototype.updateInterval = function() {
      /* chrise: removed all this pointless logic. It breaks if the first watched
       * element isn't in time-order, and an update every 60 seconds regardless
    ...
    TimeAgo.prototype.timeAgoInWords = function(timeString, suffix) {
      var absolutTime;
      absolutTime = this.parse(timeString);
      if (suffix === undefined) {
        suffix = this.options.lang.suffix;
      }
      return "" + this.options.lang.prefixes.ago + (this.distanceOfTimeInWords(absolutTime)) + suffix;
    };
    ...
    TimeAgo.prototype.getTimeDistanceInMinutes = function(absolutTime) {
      ...
      return Math.floor((Math.abs(timeDistance) / 1000) / 60);
    };
    ...
    TimeAgo.prototype.distanceOfTimeInWords = function(absolutTime) {
	  ...
	  if (dim === 0) {
        ...
        return "a " + this.options.lang.units.minute;
      } else if (dim >= 2 && dim <= 59) {
        return "" + dim + " " + this.options.lang.units.minutes;
      } else if (dim >= 60 && dim <= 89) {
        return "" + this.options.lang.prefixes.about + " an " + this.options.lang.units.hour;
      } else if (dim >= 90 && dim <= 1439) {
        return "" + this.options.lang.prefixes.about + " " + (Math.round(dim / 60)) + " " + this.options.lang.units.hours;
      } else if (dim >= 1440 && dim <= 2519) {
        return "a " + this.options.lang.units.day;
      } else if (dim >= 2520 && dim <= 43199) {
        return "" + (Math.round(dim / 1440)) + " " + this.options.lang.units.days;
      } else if (dim >= 43200 && dim <= 86399) {
        return "" + this.options.lang.prefixes.about + " a " + this.options.lang.units.month;
      } else if (dim >= 86400 && dim <= 525599) {
        return "" + (Math.round(dim / 43200)) + " " + this.options.lang.units.months;
      } else if (dim >= 525600 && dim <= 655199) {
        return "" + this.options.lang.prefixes.about + " a " + this.options.lang.units.year;
      } else if (dim >= 655200 && dim <= 914399) {
        return "" + this.options.lang.prefixes.over + " a " + this.options.lang.units.year;
